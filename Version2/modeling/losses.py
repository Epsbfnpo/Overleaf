...
class GDRNetLoss_Integrated(nn.Module):
    ...
    def forward(self, output_dict, queue_dict, labels, domains):
        ...
        lambda_sup = 1.0
        lambda_cass = 0.5
        queue_labels = queue_dict.get('queue_labels', None)
        if queue_labels is not None and (queue_labels == -1).any():
            lambda_queue = 0.0
        else:
            lambda_queue = 0.5
        loss_total = lambda_sup * (loss_sup_cnn + loss_sup_vit) + lambda_cass * loss_cass + lambda_queue * (loss_queue_cnn + loss_queue_vit)
        loss_dict = {"loss": loss_total.item(), "sup_cnn": loss_sup_cnn.item(), "sup_vit": loss_sup_vit.item(), "cass": loss_cass.item(), "queue": (lambda_queue * (loss_queue_cnn + loss_queue_vit)).item() / 2}
