...
default_cfgs = {
    ...
    'swin_base_patch4_window7_224_in22k': _cfg(
        ...
        num_classes=21841),
    'swin_base_patch4_window7_224_in22k_iw': _cfg(
        url='https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window7_224_22k.pth',
        num_classes=21841),
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape[:]
        x = x.view(B, C, int(np.sqrt(N)), -1)
        v = self.global_avgpool(x)
        return v.view(v.size(0), -1)
        # x = self.head(x)
        # return x

class SwinTransformer_IW(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.norm = norm_layer(self.num_features)
        self.norm_1 = norm_layer(256)
        self.c_mlp1 = nn.Linear(256, 256)
        self.n_mlp1 = nn.Linear(256, 256)
        self.norm_2 = norm_layer(512)
        self.c_mlp2 = nn.Linear(512, 512)
        self.n_mlp2 = nn.Linear(512, 512)
        self.norm_3 = norm_layer(1024)
        self.c_mlp3 = nn.Linear(1024, 1024)
        self.n_mlp3 = nn.Linear(1024, 1024)
        self.norm_4 = norm_layer(1024)
        self.c_mlp4 = nn.Linear(1024, 1024)
        self.n_mlp4 = nn.Linear(1024, 1024)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def instance_whitening_loss(self, x):
        ...
        f_cor = torch.bmm(f_map, f_map.transpose(1, 2)).div(HW-1).cuda() + (eps * eye)  # C X C / HW
    ...
    def semantic_whitening_loss(self, x):
        ### semantic whitening
        eps = 1e-5
        f_map = x
        B, C = f_map.shape  # i-th feature size (B X C)
        f_map = f_map.contiguous().view(B, C, 1)
        if B == 1:
            f_map = torch.cat([f_map, f_map.clone()], dim=0)
        f_map = torch.nn.InstanceNorm1d(f_map.shape[1], affine=False)(f_map) 
        if B == 1:
            f_map = f_map[0]
        # b_norm = torch.nn.BatchNorm2d(C, track_running_stats=False).cuda()
        # f_map = b_norm(f_map)
        # f_map = nn.LayerNorm(C)(f_map)
        eye = torch.eye(C).cuda()
        f_map = f_map.contiguous().view(B, C, 1)  # B X C > B X C X 1
        f_cor = torch.bmm(f_map, f_map.transpose(1, 2)) + (eps * eye)  # C X C / HW
        f_cor_masked = torch.triu(f_cor) # take upper triangular matrix
    ...
    def forward_features(self, x):
        ...
        if self.absolute_pos_embed is not None:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
        l1 = self.layers[0](x) # 256
        B, N, C = l1.shape[:]
        hw = int(np.sqrt(N))
        l1 = self.norm_1(l1)
        l1 = l1.reshape(B, -1, hw, hw)
        iw_loss += self.instance_whitening_loss(l1)
        l1 = l1.reshape(B, N, C)
        l2 = self.layers[1](l1) # 512
        B, N, C = l2.shape[:]
        hw = int(np.sqrt(N))
        l2 = self.norm_2(l2)
        l2 = l2.reshape(B, -1, hw, hw)
        iw_loss += self.instance_whitening_loss(l2)
        l2 = l2.reshape(B, N, C)
        l3 = self.layers[2](l2) # 1024
        B, N, C = l3.shape[:]
        hw = int(np.sqrt(N))
        l3 = self.norm_3(l3)
        l3 = l3.reshape(B, -1, hw, hw)
        iw_loss += self.instance_whitening_loss(l3)
        l3 = l3.reshape(B, N, C)
        l4 = self.layers[3](l3) # 1024
        B, N, C = l4.shape[:]
        hw = int(np.sqrt(N))
        l4 = self.norm_4(l4)
        l4 = l4.reshape(B, -1, hw, hw)
        iw_loss += self.instance_whitening_loss(l4)
        l4 = l4.reshape(B, N, C)
        # x = self.norm(l4)  # B L C
        # x = self.avgpool(x.transpose(1, 2))  # B C 1
        # x = torch.flatten(x, 1)
    ...
    def forward(self, x):
        ...
        l4, iw_loss = self.forward_features(x)
        B, N, C = l4.shape[:]
        x = l4.view(B, C, int(np.sqrt(N)), -1)
        v = self.global_avgpool(x)
        v = v.view(v.size(0), -1)
        # sw_loss = self.semantic_whitening_loss(v)
        # return v, iw_loss, sw_loss, l4
        return v, iw_loss, l4
        # return v, iw_loss, l4, img_mean, img_std
        # x = self.head(x)
        # return x
...
def _create_swin_transformer_iw(variant, pretrained=False, default_cfg=None, **kwargs):
    if default_cfg is None:
        default_cfg = deepcopy(default_cfgs[variant])
    overlay_external_default_cfg(default_cfg, kwargs)
    default_num_classes = default_cfg['num_classes']
    ...
    model = build_model_with_cfg(
        SwinTransformer_IW, variant, pretrained,
        default_cfg=default_cfg,
        img_size=img_size,
        num_classes=num_classes,
        pretrained_filter_fn=checkpoint_filter_fn,
...
@register_model
def swin_base_patch4_window7_224_in22k_iw(pretrained=False, **kwargs):
    """ Swin-B @ 224x224, trained ImageNet-22k
    """
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _create_swin_transformer_iw('swin_base_patch4_window7_224_in22k', pretrained=pretrained, **model_kwargs)
