...
class GDRNetLoss_Integrated(nn.Module):
    ...
    def forward(self, output_dict, labels, domains):
        ...
        proj_vit_norm = F.normalize(proj_vit.detach(), dim=-1)
        loss_proj_align_raw = 2.0 - 2.0 * (proj_cnn_norm * proj_vit_norm).sum(dim=-1)
        loss_proj_align = (loss_proj_align_raw * dcr_weight).mean()
        loss_proj_align = loss_proj_align + 0.0 * proj_vit.sum()
        loss_total = loss_sup_vit + 0.2 * loss_sup_cnn + 1.0 * loss_distill_cnn + 1.0 * loss_proj_align
        loss_dict = {"loss": loss_total.item(), "sup_vit": loss_sup_vit.item(), "sup_cnn": loss_sup_cnn.item(), "distill_cnn": loss_distill_cnn.item(), "proj_align": loss_proj_align.item()}
